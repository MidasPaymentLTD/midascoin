...
#if defined(NDEBUG)
# error "MidasCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x59f656e83431441c112216d2970d25b30cd99d44463c1cbb6a88072b1b4419f4");
...
const string strMessageMagic = "MidasCoin Signed Message:\n";
...
{
    // MidasCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = GetDistribution(nHeight) * COIN;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60 * 60; // MidasCoin: 1 hour
static const int64 nTargetSpacing = 2.5 * 60; // MidasCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_start(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
...
unsigned int static GetNextWorkRequired_base(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
...
{
    if(pindexLast->nHeight < 1160){
        return GetNextWorkRequired_start(pindexLast, pblock);
    }else{
        return GetNextWorkRequired_base(pindexLast, pblock);
    }
}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MidasCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MidasCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "NY Times 18/Sept/2014 Scottish Vote Weighs Pride Against Risk";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x6e0d159acc21f04b53fc21bb0c889f525c6e876f55600bf2b426696555d92840"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MidasCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MidascoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MidascoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MidascoinMiner(CWallet *pwallet)
{
    printf("MidascoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("midascoin-miner");
    ...
    try { loop {
        ...
        printf("Running MidascoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MidascoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MidascoinMiner, pwallet));
}
